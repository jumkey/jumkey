...
function change(){
	$("#money").val($("#feeitemid>option:selected").attr("title"));
}
//入口
$(document).ready(function(){
	...
	$(".edit").live("click",function(){
		...
		var count=$("#feeitemid option").length;
		for(var i=0;i<count;i++){
			if($("#feeitemid").get(0).options[i].text == text){
				$("#feeitemid").get(0).options[i].selected = true;
				break;
			}
		}
		
		$("#money").val($(this).parent()[0].cells[2].innerHTML);
...
<div>
    <div id="tabhead"><table>
      <tr>
        ...
        <td><span style="background: url('images/311.gif') no-repeat; padding-left: 17px; font-size:14px;" class="STYLE4">费项信息列表</span></td>
      ...
      <tr>
        ...
        <td bgcolor="e5f1d6"><table id="list" style="width: 100%;border-spacing: 1px;border-collapse: separate; background-color: #CECECE;">
          <tr class="tabTitle">
            <td width="8%">选择</td>
            <td width="18%">费项名称</td>
            <td width="18%">预计金额/元</td>
            <td width="18%">初始金额/元</td>
            <td width="18%">费项性质</td>
            <td width="10%">编辑</td>
            <td width="10%">删除</td>
          </tr>
          <s:iterator status="index" value="meetinginfo.meetingFeeItems" id="meetingfeeitem">
          <tr class="tabItem">
            <td><input name="checkbox" type="checkbox" value="${meetingfeeitem.id }" /></td>
            <td>${meetingfeeitem.feeItem.name }</td>
            <td>${meetingfeeitem.money }</td>
            <td>${meetingfeeitem.feeItem.startfee }</td>
            <td>${meetingfeeitem.feeItem.nature }</td>
            <td class="edit">[<a href="javascript:void(0);" >编辑</a>]</td>
            <td class="delete">[<a href="javascript:void(0);">删除</a>]</td>
          </tr></s:iterator>
        </table></td>
...
<form id="form1" action="/" method="post">
	<input type="hidden" name="meetingfeeitem.meetingInfo.id" value="${meetingfeeitem.meetingInfo.id }" id="meetinginfoid"/><br/>
	<input type="hidden" name="meetingfeeitem.id" value="" id="meetingfeeitemid"/><br/>
	费项：<select name="meetingfeeitem.feeItem.id" id="feeitemid" onchange="change()">
		<option value="" title="">请选择</option>
		<s:iterator status="index" value="list" id="feeitem">
		<option value="${feeitem.id }" title="${feeitem.startfee }">${feeitem.name }</option>
		</s:iterator>
	</select><br/>
	预计金额：<input type="text" name="meetingfeeitem.money" value="" id="money"/><br/>
</form>
<div id="alert"></div>
</div>

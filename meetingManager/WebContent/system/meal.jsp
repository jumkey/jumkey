...
$(document).ready(function(){
	...
	$("#dialog").dialog({
		...
		title:"编辑",position:"center",width:300,height:400,
	...
	$(".edit").live("click",function(){
		...
		$("#money").val($(this).parent()[0].cells[1].innerHTML);
		$("#number").val($(this).parent()[0].cells[5].innerHTML);
		if($(this).parent()[0].cells[2].innerHTML=="集体")
		{
			$("#type").get(0).selectedIndex=0;
		}else{
			$("#type").get(0).selectedIndex=1;
		}
		$("#drinkscost").val($(this).parent()[0].cells[6].innerHTML);
		$("#expenses").val($(this).parent()[0].cells[7].innerHTML);
		$("#mealtime").val($(this).parent()[0].cells[8].innerHTML);
		$("#dialog").dialog({
			buttons:{
				"修改":function(){
					$.post('main/domeal_modify.action', {
						"meal.id":$("#id").val(),
						"meal.date":$("#date").val(),
						"meal.address":$("#address").val(),
						"meal.number":$("#number").val(),
						"meal.type":$("#type").val(),
						"meal.money":$("#money").val(),
						"meal.drinkscost":$("#drinkscost").val(),
						"meal.expenses":$("#expenses").val(),
						"meal.mealtime":$("#mealtime").val()
					}, function(data) {
	...
	$(".delete").live("click",function(){
		...
		confirm("确认删除",function(){
			$.ajax({
				url: "main/domeal_delete.action",
				data: {
					"meal.id": $this_.parent().find("input").val()
				},
				type: "post",
				dataType: "json",
	...
	$("#delselect").click(function(){
		...
		if (selectedItems.length == 0){
			...
			confirm("确认删除",function(){
				...
				$.ajax({
					type: "POST",
					url: "main/domeal_delselect.action",
...
<div>
    <div id="tabhead"><table>
      <tr>
        ...
        <td><span style="background: url('images/311.gif') no-repeat; padding-left: 17px; font-size:14px;" class="STYLE4">用餐列表</span></td>
    ...
    <div id="tabbody" style="width: 100%;height: 100%;"><table>
      <tr>
        ...
        <td bgcolor="e5f1d6"><table id="list" style="width: 100%;border-spacing: 1px;border-collapse: separate; background-color: #CECECE;">
          ...
          <s:iterator status="index" value="list" id="meal">
          <tr class="tabItem">
            <td><input name="checkbox" type="checkbox" value="${meal.id }" /></td>
            <td>${meal.money }</td>
            <td>${meal.type==0?"集体":"个人" }</td>
            <td>${meal.date }</td>
            <td>${meal.address }</td>
            <td>${meal.number }</td>
            <td>${meal.drinkscost }</td>
            <td>${meal.expenses }</td>
            <td class="hideborder">${meal.mealtime }</td>
            <td class="edit">[<a href="javascript:void(0);" >编辑</a>]</td>
            <td class="delete">[<a href="javascript:void(0);">删除</a>]</td>
          </tr>
          </s:iterator>
...
<form id="form1" name="form1" method="post" action="">
  <input type="hidden" name="meal.id" value="" id="id"/><br/>
  <label for="address">用餐地点</label>
  <input type="text" name="meal.address" id="address" /><br />
  <label for="date">用餐时间</label>
  <input type="text" name="meal.date" id="date" /><br />
  <label for="hotel">用餐费用</label>
  <input type="text" name="meal.money" id="money" /><br />
  <label for="hotel">用餐人数</label>
  <input type="text" name="meal.number" id="number" /><br />
  <label for="hotel">用餐类型</label>
  <select name="meal.type" id="type">
  	<option value="0">集体</option>
  	<option value="1">个人</option>
  </select><br />
  <label for="drinkscost">酒水费用</label>
  <input type="text" name="meal.drinkscost" id="drinkscost" /><br />
  <label for="expenses">支出费用</label>
  <input type="text" name="meal.expenses" id="expenses" /><br />
  <label for="mealtime">用餐时段</label>
  <select name="meal.mealtime" id="mealtime">

...
function page(num){
	if(!isNaN(num)){
		location.href="<%=basePath%>admin/role_list.action?page.currentPage="+num;
	}
}
function gopage(){
	if(isNaN($("#gogo").val())){
...
$(document).ready(function(){
	...
	$("#dialog").dialog({
		...
		title:"编辑",position:"center",width:300,height:180,
	...
	$(".edit").live("click",function(){
		...
		//设置form数据
		$("#roleid").val($(this).parent()[0].cells[1].innerHTML);
		$("#name").val($(this).parent()[0].cells[2].innerHTML);
		$("#dialog").dialog({
			buttons:{
				"修改":function(){
					$.post('admin/jrole_modify.action', {
						"role.id":$("#roleid").val(),
						"role.name":$("#name").val()
					}, function(data) {
	...
	$(".delete").live("click",function(){
		...
		confirm("确认删除",function(){
			$.ajax({
				url: "admin/jrole_delete.action",
				data: {
					"role.id": $this_.parent().find("input").val()
				},
				type: "post",
				dataType: "json",
	...
	$("#delselect").click(function(){
		...
		if (selectedItems.length == 0){
			...
			confirm("确认删除",function(){
				$.ajax({
					type: "POST",
					url: "admin/jrole_delselect.action",
...
<div>
    <div id="tabhead"><table>
      <tr>
        ...
        <td><span style="background: url('images/311.gif') no-repeat; padding-left: 17px; font-size:14px;" class="STYLE4">角色列表</span></td>
    ...
    <div id="tabbody" style="width: 100%;height: 100%;"><table>
      <tr>
        ...
        <td bgcolor="e5f1d6"><table id="list" style="width: 100%;border-spacing: 1px;border-collapse: separate; background-color: #CECECE;">
          ...
          <s:iterator status="index" value="list" id="role">
          <tr class="tabItem">
            <td><input name="checkbox" type="checkbox" value="${role.id }" /></td>
            <td>${role.id }</td>
            <td>${role.name }</td>
            <td><s:date name="#role.createtime" format="yyyy-MM-dd HH:ss" /></td>
            <td class="definelimit">[<a href="admin/rolelimit_list.action?roleId=${role.id }" >权限</a>]</td>
            <td class="edit">[<a href="javascript:void(0);" >编辑</a>]</td>
            <td class="delete">[<a href="javascript:void(0);">删除</a>]</td>
          </tr></s:iterator>
        </table></td>
...
<form id="form1" action="/" method="post">
	<input type="hidden" name="role.id" value="" id="roleid"/><br/>
	角色名称：<input type="text" name="role.name" value="" id="name"/><br/>
</form>
<div id="alert"></div>
</div>

...
function page(num,id){
	if(!isNaN(num)&&!isNaN(id)){
		location.href="<%=basePath%>main/joinactivities_list.action?page.currentPage="+num+"&id="+id;
	}
}
function gopage(){
	if(isNaN($("#gogo").val())){
...
$(document).ready(function(){
	...
	$("#dialog").dialog({
		...
		title:"编辑",position:"center",width:350,height:330,
	...
	$(".edit").live("click",function(){
		...
		//设置form数据		
		$("#joinactivitiesid").val($(this).parent().find("input").val());
		$("#expendamount").val($(this).parent()[0].cells[4].innerHTML);
		$("#expenditures").val($(this).parent()[0].cells[5].innerHTML);
		$("#dialog").dialog({
			buttons:{
				"修改":function(){
					$.post('main/dojoinactivities_modify.action', {
					    "joinactivities.id":$("#joinactivitiesid").val(),
						"joinactivities.expendamount":$("#expendamount").val(),
						"joinactivities.expenditures":$("#expenditures").val()
					}, function(data) {
	...
	$(".delete").live("click",function(){
		...
		confirm("确认删除",function(){
			$.ajax({
				url: "main/dojoinactivities_delete.action",
				data: {
					"joinactivities.id": $this_.parent().find("input").val()
				},
				type: "post",
				dataType: "json",
	...
	$("#delselect").click(function(){
		...
		if (selectedItems.length == 0){
			...
			confirm("确认删除",function(){
				...
				$.ajax({
					type: "POST",
					url: "main/dojoinactivities_delselect.action",
	...
	$("#add").bind("click",function(){
		$("#joinactivitiesid").val("");
		$("#expendamount").val("");
		$("#expenditures").val("");
		$("#dialog").dialog({
			buttons:{
				"新增":function(){
					$.post('main/dojoinactivities_add.action', {
					    "joinactivities.activities.id":${id },
						// "joinactivities.user.id":$("#userid").val(),
						"joinactivities.expendamount":$("#expendamount").val(),
						"joinactivities.expenditures":$("#expenditures").val()
					}, function(data) {
						//alert(data);
...
<div>
    <div id="tabhead"><table>
      <tr>
        ...
        <td><span style="background: url('images/311.gif') no-repeat; padding-left: 17px; font-size:14px;" class="STYLE4">活动参加人员列表</span></td>
    ...
    <div id="tabbody" style="width: 100%;height: 100%;"><table>
      <tr>
        ...
        <td bgcolor="e5f1d6"><table id="list" style="width: 100%;border-spacing: 1px;border-collapse: separate; background-color: #CECECE;">
          ...
          <s:iterator status="index" value="list" id="join">
          <s:if test="#join.user.usertype=='家属'">
          <tr class="tabItem child">
            <td><input name="checkbox" type="checkbox" value="${join.id }" /></td>
            <td>${join.id }</td>
            <td>${join.user.name }</td>
            <td>${join.activities.name }</td>
            <td>${join.expendamount}</td>
            <td>${join.expenditures}</td>
            <td>${join.user.user.name }的家属</td>
            <td class="edit">[<a href="javascript:void(0);" >编辑</a>]</td>
            <td class="delete">[<a href="javascript:void(0);">删除</a>]</td>
          </tr>
          </s:if>
...
<form id="form1" name="form1" method="post" action="">
	<input type="hidden" name="joinactivities.id" value="" id="joinactivitiesid"/><br/>
	<label for="userid">用户编号</label>
	<input type="text" name="joinactivities.user.id" value="" id="userid" disabled="disabled"/><br/>
	<label for="username">用户姓名</label>
	<input type="text" name="user.name" value="" id="username"/><br/>
	<span id="uu"></span>
	...
	<script type="text/javascript">
		...
		function checkusername(){
			$.post('main/dojoinactivities_checkusername.action', {
				"id":${id},
	...
	<label for="expendamount">缴费金额</label>
  	<input type="text" name="joinactivities.expendamount" id="expendamount" /><br />
  	<label for="expenditures">支出金额</label>
  	<input type="text" name="joinactivities.expenditures" id="expenditures" /><br />
</form>
<div id="alert"></div>
</div>
</div><!--end tabwrap-->

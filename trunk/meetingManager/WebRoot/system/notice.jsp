	...
	$(".delete").live("click",function(){
		...
		confirm("确认删除",function(){
			$.ajax({
				...
				data: {
					"price.id": $this_.parent().find("input").val()
				},
				type: "post",
				dataType: "json",
			...
			buttons:{
				"新增":function(){
					$.post('main/jnotice_add.action', {
						...
						"price.units.name":$("#unitsname").val(),
						"price.date":$("#date").val(),
						"price.address":$("#address").val(),
						"price.hotel":$("#hotel").val(),
						"meeting.title":$("#title").val(),
						"meeting.author":$("#author").val(),
						"meeting.desc1":$("#desc1").val()
					}, function(data) {
						//alert(data);
						var result=$.parseJSON(data.result);
      ...
      <tr>
        ...
        <td bgcolor="e5f1d6"><table id="list" style="width: 100%;border-spacing: 1px;border-collapse: separate; background-color: #CECECE;">
          <tr class="tabTitle">
            <td width="20%">选择</td>
            <td width="20%">会议标题</td>
            <td width="20%">计划通知日期</td>
            <td width="20%">计划通知人数</td>
            <td width="20%">操作</td>
          </tr>
          <s:iterator status="index" value="list" id="notice">
          <tr class="tabItem">
            <td><input name="checkbox" type="checkbox" value="${notice.id }" /></td>
            <td>${notice.meetingInfo.title }</td>
            <td>${notice.date }</td>
            <td>${notice.number }</td>
            <td>[<a href="main/notice_send.action?notice.id=${notice.id }" >发送/回执通知</a>]</td>
          </tr></s:iterator>
        </table></td>
...
<form id="form1" name="form1" method="post" action="">
  <input type="hidden" name="notice.meetingInfo.id" value="" id="meetingInfoid"/><br/>
  <label for="number">计划通知人数</label>
  <input type="text" name="notice.number" id="number" /><br />
  <label for="date">计划通知日期</label>
  <input type="text" name="notice.date" id="date" /><br />
</form>
<div id="alert"></div>
</div>

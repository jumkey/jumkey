...
$(document).ready(function(){
	...
	$(".edit").live("click",function(){
		...
		//设置form数据
		$("#feeitemid").val($this_.parent().find("input").val());
		$("#name").val($(this).parent()[0].cells[1].innerHTML);
		$("#type").val($(this).parent()[0].cells[2].innerHTML);
		$("#startfee").val($(this).parent()[0].cells[3].innerHTML);
		$("#nature").val($(this).parent()[0].cells[4].innerHTML);

		$("#dialog").dialog({
			buttons:{
				"修改":function(){
					$.post('main/jfeeitem_modify.action', {
						"feeitem.id":$("#feeitemid").val(),
						"feeitem.name":$("#name").val(),
						"feeitem.type":$("#type").val(),
						"feeitem.startfee":$("#startfee").val(),
						"feeitem.nature":$("#nature").val()
					}, function(data) {
	...
	$(".delete").live("click",function(){
		...
		confirm("确认删除",function(){
			$.ajax({
				url: "main/jfeeitem_delete.action",
				data: {
					"feeitem.id": $this_.parent().find("input").val()
				},
				type: "post",
				dataType: "json",
	...
	$("#delselect").click(function(){
		...
		if (selectedItems.length == 0){
			...
			confirm("确认删除",function(){
				...
				$.ajax({
					type: "POST",
					url: "main/jfeeitem_delselect.action",
...
<div>
    <div id="tabhead"><table>
      <tr>
        ...
        <td><span style="background: url('images/311.gif') no-repeat; padding-left: 17px; font-size:14px;" class="STYLE4">费项列表</span></td>
    ...
    <div id="tabbody" style="width: 100%;height: 100%;"><table>
      <tr>
        ...
        <td bgcolor="e5f1d6"><table id="list" style="width: 100%;border-spacing: 1px;border-collapse: separate; background-color: #CECECE;">
          ...
          <tr class="tabItem">
            <td><input name="checkbox" type="checkbox" value="${feeitem.id }" /></td>
            <td>${feeitem.name }</td>
            <td>${feeitem.type }</td>
            <td>${feeitem.startfee }</td>
            <td>${feeitem.nature }</td>
            <td class="edit">[<a href="javascript:void(0);" >编辑</a>]</td>
            <td class="delete">[<a href="javascript:void(0);">删除</a>]</td>
          </tr></s:iterator>
        </table></td>
...
<form id="form1" action="/" method="post">
	<input type="hidden" name="feeitem.id" value="" id="feeitemid"/><br/>
	名称：<input type="text" name="feeitem.name" value="" id="name"/><br/>
	类型：<input type="text" name="feeitem.type" value="" id="type"/><br/>
	初始值：<input type="text" name="feeitem.startfee" value="" id="startfee"/><br/>
	性质：<select name="feeitem.nature" id="nature">

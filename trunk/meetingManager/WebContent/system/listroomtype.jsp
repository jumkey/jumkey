...
function page(num){
	if(!isNaN(num)){
		location.href="<%=basePath%>main/roomtype_list.action?page.currentPage="+num;
	}
}
function gopage(){
	if(isNaN($("#gogo").val())){
...
$(document).ready(function(){
	...
	$(".edit").live("click",function(){
		...
		//设置form数据
		$("#roomtypeid").val($(this).parent()[0].cells[1].innerHTML);
		$("#type").val($(this).parent()[0].cells[2].innerHTML);
		$("#dialog").dialog({
			buttons:{
				"修改":function(){
					$.post('main/jroomtype_modify.action', {
						"roomtype.id":$("#roomtypeid").val(),
						"roomtype.type":$("#type").val(),
					}, function(data) {
	...
	$(".delete").live("click",function(){
		...
		confirm("确认删除",function(){
			$.ajax({
				url: "main/jroomtype_delete.action",
				data: {
					"roomtype.id": $this_.parent().find("input").val()
				},
				type: "post",
				dataType: "json",
		...
		$("#dialog").dialog({
			buttons:{
				"新增":function(){
					...
					$.post('main/jroomtype_add.action', {
						"roomtype.type":$("#type").val(),
					}, function(data) {
						var result=$.parseJSON(data.result);
						if(result.success!="true"){
	...
	$("#delselect").click(function(){
		...
		if (selectedItems.length == 0){
			...
			confirm("确认删除",function(){
				$.ajax({
					type: "POST",
					url: "main/jroomtype_delselect.action",
...
<div>
    <div id="tabhead"><table>
      <tr>
        ...
        <td><span style="background: url('images/311.gif') no-repeat; padding-left: 17px; font-size:14px;" class="STYLE4">房间类型<br /></span></td>
    ...
    <div id="tabbody" style="width: 100%;height: 100%;"><table>
      <tr>
        ...
        <td bgcolor="e5f1d6"><table id="list" style="width: 100%;border-spacing: 1px;border-collapse: separate; background-color: #CECECE;">
          ...
          <s:iterator status="index" value="list" id="roomtype">
          <tr class="tabItem">
            <td><input name="checkbox" type="checkbox" value="${roomtype.id }" /></td>
            <td>${roomtype.id }</td>
            <td>${roomtype.type }</td>
             <td>${roomtype.sysaccount.account }</td>
            <td class="edit">[<a href="javascript:void(0);" >编辑</a>]</td>
            <td class="delete">[<a href="javascript:void(0);">删除</a>]</td>
          </tr></s:iterator>
        </table></td>
...
<form id="form1" action="/" method="post">
	<input type="hidden" name="roomtype.id" value="" id="roomtypeid"/><br/>
	类型名称：<input type="text" name="roomtype.type" value="" id="type"/><br/>
	<br/>
</form>
<div id="alert"></div>
</div>
